...
export default function AuctionForm({auction}: Props) {
  ...
  return (
    <form className='flex flex-col mt-3' onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='grid drid-cols-2 gap-3'>
        ...
        <>
          ...
          <div className='grid drid-cols-2 gap-3'>
              <Input label='Reserve Prize (enter 0 if no reserve)' name='reservePrize' control={control} type='number' rules={{required: 'Reserve Prize is required'}} />
              <DateInput label='Auction end date/time' name='auctionEnd' control={control} type='date' dateFormat='dd MMMM yyyy h:mm a' showTimeSelect rules={{required: 'Auction end date is required'}} />
          </div>
        </>}
        <div className='flex justify-between'>
            <Button outline color='gray'>Cancel</Button>
            <Button isProcessing={isSubmitting} disabled={!isValid} type='submit' outline color='success'>Save</Button>
        </div>
    </form>
  )
}
